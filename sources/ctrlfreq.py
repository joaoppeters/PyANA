...
def freqgerlim(
    ...
    for idx, value in powerflow.dgeraDF.iterrows():
        ...
        powerflow.dgerorder[idx] = powerflow.dbarraDF['nome'][
            value['numero'] - 1
        ]
...
def freqres(
    ...
    for idx, value in powerflow.dbarraDF.iterrows():
        if value['tipo'] != 0:
            ...
            powerflow.deltaP[idx] -= (
                value['demanda_ativa'] / powerflow.options['BASE']
            )
            ...
            if (
                powerflow.solution['freq'] >= powerflow.freqger['max'][nger]
            ) or (
    ...
    powerflow.deltaY = append(
        powerflow.deltaY, powerflow.deltaTger
    )
...
def freqsubjac(
    ...
    if powerflow.controldim != 0:
        powerflow.extrarowp = zeros(
            [powerflow.nger, powerflow.controldim]
        )
        powerflow.extrarowq = zeros(
            [powerflow.nger, powerflow.controldim]
        )
        powerflow.extrarowy = zeros(
            [powerflow.nger, powerflow.controldim]
        )

        powerflow.extracolp = zeros(
            [powerflow.controldim, powerflow.nger]
        )
        powerflow.extracolq = zeros(
            [powerflow.controldim, powerflow.nger]
        )
        powerflow.extracoly = zeros(
            [powerflow.controldim, powerflow.nger]
        )
        ...
        powerflow.jacob = concatenate(
            (
                ...
                concatenate(
                    (
                        concatenate(
                            (powerflow.ypt, powerflow.ypv), axis=1
                        ),
                        concatenate(
                            (powerflow.yqt, powerflow.yqv), axis=1
                        ),
                        concatenate(
                            (powerflow.yxt, powerflow.yxv), axis=1
                        ),
                    ),
                    axis=0,
                ),
