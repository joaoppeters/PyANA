...
class Freq:
    ...
    def freqsol(
        ...
        if (powerflow.setup.control['FREQ']) and (powerflow.setup.codes['DGER']):
            # Variáveis
            powerflow.solution['active_generation'] = zeros(powerflow.setup.nger)
            powerflow.solution['qlim_reactive_generation'] = zeros(powerflow.setup.nger)
            ...
            for idx, value in powerflow.setup.dbarraDF.iterrows():
                ...
                if value['tipo'] != 0:
                    powerflow.solution['active_generation'][nger] = (
                        value['potencia_ativa'] / powerflow.setup.options['BASE']
                    )
                    powerflow.solution['qlim_reactive_generation'][nger] = (
                        value['potencia_reativa'] / powerflow.setup.options['BASE']
                    )
                    nger += 1
    ...
    def freqgerlim(
        ...
        for idx, value in powerflow.setup.dgeraDF.iterrows():
            ...
            powerflow.setup.dgerorder[idx] = powerflow.setup.dbarraDF['nome'][
                value['numero'] - 1
            ]
            # Frequência máxima gerador `idx`
            powerflow.setup.freqger['max'][idx] = (
                powerflow.setup.fesp
                + value['estatismo']
                * 1e-2
                * (
                    powerflow.setup.dbarraDF['potencia_ativa'][value['numero'] - 1]
                    - value['potencia_ativa_minima']
                )
                / powerflow.setup.options['BASE']
            )
            # Frequência mínima gerador `idx`
            powerflow.setup.freqger['min'][idx] = (
                powerflow.setup.fesp
                + value['estatismo']
                * 1e-2
                * (
    ...
    def freqsch(
        ...
        powerflow.pqsch['potencia_ativa_gerada_especificada'] = zeros(
            powerflow.setup.nger
        )
        powerflow.pqsch['potencia_reativa_gerada_especificada'] = zeros(
            powerflow.setup.nger
        )
        powerflow.pqsch['magnitude_tensao_especificada'] = zeros(
            powerflow.setup.nbus
        )
        powerflow.pqsch['defasagem_angular_especificada'] = zeros(
            powerflow.setup.nbus
        )
        ...
        for idx, value in powerflow.setup.dbarraDF.iterrows():
            if value['tipo'] != 0:
                ...
                powerflow.pqsch['potencia_ativa_gerada_especificada'][
                    nger
                ] = value['potencia_ativa']
                # Potência reativa gerada
                powerflow.pqsch['potencia_reativa_gerada_especificada'][
                    nger
                ...
                if value['tipo'] == 2:
                    # Defasagem angular
                    powerflow.pqsch['defasagem_angular_especificada'][
                        idx
                    ] = radians(value['angulo'])
                # Incrementa contador
                nger += 1
        ...
        powerflow.pqsch[
            "potencia_reativa_gerada_especificada"
        ] /= powerflow.setup.options['BASE']
    ...
    def freqres(
        ...
        ## Inicialização
        # Vetor de resíduos
        powerflow.deltaPger = zeros([powerflow.setup.nger])
        powerflow.deltaQger = zeros([powerflow.setup.nger])
        ...
        for idx, value in powerflow.setup.dbarraDF.iterrows():
            if value['tipo'] != 0:
                ...
                powerflow.deltaP[idx] = powerflow.solution['active_generation'][
                    nger
                ]
                powerflow.deltaP[idx] -= (
                    value['demanda_ativa'] / powerflow.setup.options['BASE']
                )
                ...
                powerflow.deltaQ[idx] = powerflow.solution['qlim_reactive_generation'][
                    nger
                ]
                powerflow.deltaQ[idx] -= (
                    value['demanda_reativa'] / powerflow.setup.options['BASE']
                )
                ...
                if (
                    powerflow.solution['freq'] >= powerflow.setup.freqger['max'][nger]
                ) or (
                    powerflow.solution['freq'] <= powerflow.setup.freqger['min'][nger]
                ):
                    ...
                    powerflow.deltaPger[nger] -= powerflow.solution[
                        "active_generation"
                    ][nger]
                    powerflow.deltaPger[nger] -= (
                        1 / (powerflow.setup.dgeraDF['estatismo'][nger] * 1e-2)
                    ) * (powerflow.solution['freq'] - powerflow.setup.fesp)
                ...
                if value['tipo'] == 2:
                    ...
                    powerflow.setup.deltaTger += powerflow.pqsch[
                        "defasagem_angular_especificada"
                    ][idx]
                    powerflow.setup.deltaTger -= powerflow.solution['theta'][idx]
        ...
        powerflow.deltaY = append(
            powerflow.deltaY, powerflow.deltaPger
        )
        powerflow.deltaY = append(
            powerflow.deltaY, powerflow.deltaQger
        )
        powerflow.deltaY = append(
            powerflow.deltaY, powerflow.setup.deltaTger
        )
    ...
    def freqsubjac(
        ...
        # Variável
        powerflow.setup.dimprefreq = deepcopy(powerflow.jacob.shape[0])
        ...
        if powerflow.setup.freqjcount == 0:
            ...
            # Submatrizes
            powerflow.setup.pxp = zeros(
                [powerflow.setup.nbus, powerflow.setup.nger]
            )  # -> APG
            ...
            powerflow.setup.ypp = zeros(
                [powerflow.setup.nger, powerflow.setup.nger]
            )  # -> CPG
            powerflow.setup.ypq = zeros([powerflow.setup.nger, powerflow.setup.nger])
            powerflow.setup.ypx = zeros(
                [powerflow.setup.nger, powerflow.setup.nare]
            )  # -> CF

            powerflow.setup.yqt = zeros([powerflow.setup.nger, powerflow.setup.nbus])
            powerflow.setup.yqv = zeros(
                [powerflow.setup.nger, powerflow.setup.nbus]
            )  # -> EQG
            ...
            powerflow.setup.yxt = zeros(
                [powerflow.setup.nare, powerflow.setup.nbus]
            )  # -> FT
            ...
            for idx, value in powerflow.setup.dbarraDF.iterrows():
                if value['tipo'] != 0:
                    powerflow.setup.pxp[idx, nger] = -1.0
                    powerflow.setup.qxq[idx, nger] = -1.0
                    powerflow.setup.yqv[nger, idx] = 1.0
                    nger += 1

                    if value['tipo'] == 2:
                        powerflow.setup.yxt[nare, idx] = 1.0

            # Submatrizes YPP YPX
            for idx, value in powerflow.setup.dgeraDF.iterrows():
                powerflow.setup.ypp[idx, idx] = 1.0
                powerflow.setup.ypx[idx, nare] = 1.0 / (value['estatismo'] * 1e-2)

        ## Montagem Jacobiana
        # Condição
        if powerflow.setup.controldim != 0:
            powerflow.setup.extrarowp = zeros(
                [powerflow.setup.nger, powerflow.setup.controldim]
            )
            powerflow.setup.extrarowq = zeros(
                [powerflow.setup.nger, powerflow.setup.controldim]
            )
            powerflow.setup.extrarowy = zeros(
                [powerflow.setup.nger, powerflow.setup.controldim]
            )

            powerflow.setup.extracolp = zeros(
                [powerflow.setup.controldim, powerflow.setup.nger]
            )
            powerflow.setup.extracolq = zeros(
                [powerflow.setup.controldim, powerflow.setup.nger]
            )
            powerflow.setup.extracoly = zeros(
                [powerflow.setup.controldim, powerflow.setup.nger]
            )
            ...
            powerflow.jacob = concatenate(
                (
                    ...
                    concatenate(
                        (
                            concatenate(
                                (powerflow.setup.ypt, powerflow.setup.ypv), axis=1
                            ),
                            concatenate(
                                (powerflow.setup.yqt, powerflow.setup.yqv), axis=1
                            ),
                            concatenate(
                                (powerflow.setup.yxt, powerflow.setup.yxv), axis=1
                            ),
                        ),
                        axis=0,
                    ),
    ...
    def frequpdt(
        ...
        powerflow.solution['active_generation'] += powerflow.setup.statevar[
            (powerflow.setup.dimprefreq) : (
                powerflow.setup.dimprefreq + powerflow.setup.nger
            )
        ]
        # Atualização da potência reativa gerada
        powerflow.solution['qlim_reactive_generation'] += powerflow.setup.statevar[
            (powerflow.setup.dimprefreq + powerflow.setup.nger) : (
                powerflow.setup.dimprefreq + 2 * powerflow.setup.nger
            )
        ]
        # Atualização da defasagem angular
        powerflow.solution['freq'] += powerflow.setup.statevar[
            (powerflow.setup.dimprefreq + 2 * powerflow.setup.nger)
        ]

        # Tratamento de limite de potência ativa
        for idx, value in powerflow.setup.dgeraDF.iterrows():
            if powerflow.solution['freq'] >= powerflow.setup.freqger['max'][idx]:
                powerflow.solution['active_generation'][idx] = (
                    value['potencia_ativa_minima'] / powerflow.setup.options['BASE']
                )
                powerflow.setup.ypp[idx][idx] = infty
            elif powerflow.solution['freq'] <= powerflow.setup.freqger['min'][idx]:
                powerflow.solution['active_generation'][idx] = (
                    value['potencia_ativa_maxima'] / powerflow.setup.options['BASE']
                )
                powerflow.setup.ypp[idx][idx] = infty
            else:
    ...
    def freqcorr(
        ...
        ## Inicialização
        # Variável
        powerflow.solution['freq'] = deepcopy(powerflow.case[case]['p']['freq'])
        powerflow.solution['active_generation'] = deepcopy(
            powerflow.case[case]['p']['active_generation']
        )
