...
class Options:
    ...
    def __init__(
        ...
        self.stdpf = {
            ...
            "SIGA": 1e-6,  # tolerância da chave sigmóide SVC-A 
            "SIGQ": 1e-6,  # tolerância da chave sigmóide SVC-Q e SVC-I, QLIM
            "SIGV": 1e-6,  # tolerância da chave sigmóide referente à variável de tensão
        ...
        for k, v in self.stdpf.items():
            if k not in setup.dcteDF['constante'].unique():
                setup.options[k] = v
            else:
                setup.options[k] = deepcopy(
                    setup.dcteDF.loc[setup.dcteDF['constante'] == k]['valor_constante'].values[
                        0
                    ]
                )

        if setup.codes['DINC']:
            setup.options['LMBD'] = setup.dincDF.loc[
