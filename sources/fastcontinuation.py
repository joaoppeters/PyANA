...
class FastContinuation:
    ...
    def fastcontinuationpowerflow(
        ...
        powerflow.setup.jdim = powerflow.jacob.shape[0]
    ...
    def cpfloop(
        ...
        while all((powerflow.solution['voltage'] >= 0.0)) and (
            ...
            if (powerflow.solution['convergence'] == "SISTEMA CONVERGENTE") and (
                ...
                if powerflow.cpfsolution['varstep'] == "volt":
                    print(
                        ...
                        * (
                            (1 / powerflow.setup.options['FDIV'])
            ...
            if (not powerflow.setup.options['FULL']) and (powerflow.cpfsolution['pmc']):
                break
    ...
    def correction(
        ...
        elif ((powerflow.solution['iter'] >= powerflow.setup.options['ACIT'])) and (
            self.case == 1
        ):
        ...
        if stage == 'p':
            ...
            if powerflow.cpfsolution['varstep'] == "lambda":
                if not powerflow.cpfsolution['pmc']:
                    powerflow.deltaPQY[-1] = powerflow.setup.options[
                        "LMBD"
            ...
            if powerflow.cpfsolution['varstep'] == "lambda":
                ...
                powerflow.deltaY = array(
                    [
                        ...
                        - powerflow.solution['voltage'][powerflow.setup.nodevarvolt]
                    ]
                )
    ...
    def exjac(
        ...
        for idx, value in powerflow.setup.dbarraDF.iterrows():
            if value['tipo'] != 2:
                ...
                if value['tipo'] == 0:
                    colarray[(idx + powerflow.setup.nbus), 0] = powerflow.cpfsolution[
                        "demanda_reativa"
                    ][idx]
    ...
    def update_statevar(
        ...
        if stage == 'p':
            ...
            if powerflow.cpfsolution['varstep'] == "lambda":
                ...
                powerflow.cpfsolution['stepsch'] += powerflow.setup.statevar[-1]

            elif powerflow.cpfsolution['varstep'] == "volt":
                powerflow.cpfsolution['step'] += powerflow.setup.statevar[-1]
                powerflow.cpfsolution['stepsch'] += powerflow.setup.statevar[-1]
                powerflow.cpfsolution['vsch'] = (
                    powerflow.solution['voltage'][powerflow.setup.nodevarvolt]
    ...
    def eigensens(
        ...
        try:
            ...
            rightvalues, rightvector = eig(
                powerflow.jacob[powerflow.setup.mask, :][:, powerflow.setup.mask]
            )
            powerflow.PF = zeros(
                [
                    powerflow.jacob[powerflow.setup.mask, :][
        ...
        if (self.varlambda > self.varvolt) and (
            ...
            if powerflow.cpfsolution['pmc']:
                if (
                    ...
                    powerflow.cpfsolution['varstep'] = "lambda"
                    powerflow.setup.options['LMBD'] = deepcopy(
    ...
    def heuristics(
        ...
        if self.case > 0:
            ...
            if (
                (
                    powerflow.solution['voltage'][powerflow.setup.slackidx]
                    < (
            ...
            if powerflow.setup.controlcount > 0:
                ...
                if (powerflow.setup.bifurcation) and (not powerflow.cpfsolution['pmc']):
                    powerflow.cpfsolution['pmc'] = True
                    powerflow.setup.pmcidx = deepcopy(self.case)
                    powerflow.cpfsolution['varstep'] = "volt"
                    powerflow.cpfsolution['div'] = 0
