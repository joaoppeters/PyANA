...
def svcres(
    ...
    powerflow.deltaY = append(
        powerflow.deltaY, powerflow.nbusdeltaSVC
    )
...
def svcsubjac(
    ...
    # Submatrizes
    powerflow.nbuspx = zeros([powerflow.nbusnbus, powerflow.nbusncer])
    powerflow.nbusqx = zeros([powerflow.nbusnbus, powerflow.nbusncer])
    powerflow.nbusyx = zeros([powerflow.nbusncer, powerflow.nbusncer])
    powerflow.nbusyt = zeros([powerflow.nbusncer, powerflow.nbusnbus])
    powerflow.nbusyv = zeros([powerflow.nbusncer, powerflow.nbusnbus])
    ...
    for idx, value in powerflow.nbusdcerDF.iterrows():
        ...
        if value['barra'] != value['barra_controlada']:
            ...
            powerflow.nbusyv[ncer, idxcer] = (
                powerflow.nbusdiffsvc[idxcer][0]
                + powerflow.nbusdiffsvc[idxcer][1]
            )
    ...
    if powerflow.nbuscontroldim != 0:
        powerflow.nbusextrarow = zeros(
            [powerflow.nbusnger, powerflow.nbuscontroldim]
        )
        powerflow.nbusextracol = zeros(
            [powerflow.nbuscontroldim, powerflow.nbusnger]
        )
        ...
        ytv = csc_matrix(
            concatenate((powerflow.nbusyt, powerflow.nbusyv), axis=1)
        )
        pqyx = csc_matrix(
            concatenate(
                (powerflow.nbuspx, powerflow.nbusqx, powerflow.nbusyx), axis=0
            )
        )
