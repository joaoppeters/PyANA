...
class Smooth:
    ...
    def qlimssmooth(
        ...
        if (
            powerflow.setup.qlimkeys.get(powerflow.setup.dbarraDF.loc[idx, 'nome'])
            is None
        ):
        ...
        if (
            case
            not in powerflow.setup.qlimkeys[powerflow.setup.dbarraDF.loc[idx, 'nome']]
        ):
            powerflow.setup.qlimkeys[powerflow.setup.dbarraDF.loc[idx, 'nome']][
                case
            ] = list()
        ...
        var = {
            ...
            qmax: powerflow.setup.dbarraDF.loc[idx, 'potencia_reativa_maxima']
            / powerflow.setup.options['BASE'],
            qmin: powerflow.setup.dbarraDF.loc[idx, 'potencia_reativa_minima']
            / powerflow.setup.options['BASE'],
        }
        ...
        powerflow.setup.qlimkeys[powerflow.setup.dbarraDF.loc[idx, 'nome']][
            case
        ].append(array([ch1.subs(var), ch2.subs(var), ch3.subs(var), ch4.subs(var)]))
    ...
    def qlimnsmooth(
        ...
        vesp = powerflow.setup.dbarraDF.loc[idx, 'tensao'] * 1e-3
        qmax = (
            powerflow.setup.dbarraDF.loc[idx, 'potencia_reativa_maxima']
            / powerflow.setup.options['BASE']
        )
        qmin = (
            powerflow.setup.dbarraDF.loc[idx, 'potencia_reativa_minima']
            / powerflow.setup.options['BASE']
        )
    ...
    def svcreactivesmooth(
        ...
        if (
            powerflow.setup.svckeys.get(powerflow.setup.dbarraDF.loc[idxcer, 'nome'])
            is None
        ):
            powerflow.setup.svckeys[
                powerflow.setup.dbarraDF.loc[idxcer, 'nome']
            ] = dict()

        if (
            case
            not in powerflow.setup.svckeys[powerflow.setup.dbarraDF.loc[idxcer, 'nome']]
        ):
            powerflow.setup.svckeys[powerflow.setup.dbarraDF.loc[idxcer, 'nome']][
                case
            ] = list()
        ...
        varkey = {
            ...
            bmin: powerflow.setup.dcerDF.loc[ncer, 'potencia_reativa_minima']
            / (
                powerflow.setup.options['BASE']
                * (powerflow.setup.dbarraDF.loc[idxcer, 'tensao_base'] * 1e-3) ** 2
            ),
            bmax: powerflow.setup.dcerDF.loc[ncer, 'potencia_reativa_maxima']
            / (
                powerflow.setup.options['BASE']
                * (powerflow.setup.dbarraDF.loc[idxcer, 'tensao_base'] * 1e-3) ** 2
            ),
        ...
        # Expressão Geral
        powerflow.setup.diffsvc[idxcer] = array(
            [
                diffyvk.subs(var),
                diffyvm.subs(var),
                diffyqgk.subs(var),
    ...
    def svccurrentsmooth(
        ...
        varkey = {
            ...
            / (
                powerflow.setup.options['BASE']
                * powerflow.setup.dbarraDF.loc[idxcer, 'tensao_base']
                * 1e-3
            ),
            bmax: powerflow.setup.dcerDF.loc[ncer, 'potencia_reativa_maxima']
            / (
    ...
    def svcalphasmooth(
        ...
        vmsch = powerflow.setup.dbarraDF.loc[idxctrl, 'tensao'] * 1e-3
        vmmax = vmsch + (
            powerflow.setup.dcerDF.loc[ncer, 'droop']
            * powerflow.setup.alphabeq.subs(alpha, pi / 2)
            * (powerflow.solution['voltage'][idxcer] ** 2)
        )
        vmmin = vmsch + (
            powerflow.setup.dcerDF.loc[ncer, 'droop']
            * powerflow.setup.alphabeq.subs(alpha, pi)
            * (powerflow.solution['voltage'][idxcer] ** 2)
        )
        ...
        ch1 = 1 / (
            1
            + npexp(
                powerflow.setup.options['SIGK']
                * (powerflow.solution['alpha'] - alphaliminf)
            )
        )
        ...
        ch2 = 1 / (
            ...
            + npexp(
                -powerflow.setup.options['SIGK']
                * (powerflow.solution['alpha'] - alphalimsup)
            )
        )
        ...
        ) * (-vmsch - (r * (vk**2) * powerflow.setup.alphabeq) + vm)
        ...
        if powerflow.solution['alpha'] <= pi / 2 + powerflow.setup.options['SIGA']:
        ...
        elif powerflow.solution['alpha'] >= pi - powerflow.setup.options['SIGA']:
    ...
    def qlimstorage(
        ...
        if powerflow.method == "CPF":
            ...
            for busname, _ in powerflow.setup.qlimkeys.items():
                # Variáveis
                busidx = powerflow.setup.dbarraDF.index[
                    powerflow.setup.dbarraDF['nome'] == busname
                ].tolist()[0]

                qmax = powerflow.setup.dbarraDF.loc[
                    powerflow.setup.dbarraDF['nome'] == busname,
                    'potencia_reativa_maxima',
                ].values[0]
                qmin = powerflow.setup.dbarraDF.loc[
                    powerflow.setup.dbarraDF['nome'] == busname,
                    'potencia_reativa_minima',
                ].values[0]
                vesp = (
                    powerflow.setup.dbarraDF.loc[
                        powerflow.setup.dbarraDF['nome'] == busname, 'tensao'
                    ].values[0]
                    * 1e-3
                )

                ch1space = linspace(
                    start=(qmax - (powerflow.setup.options['SIGQ'] * 1e1)),
                    stop=(qmax + (powerflow.setup.options['SIGQ'] * 1e1)),
                    num=10000,
                    endpoint=True,
                )
                ch1value = 1 / (
                    ...
                    + npexp(
                        -powerflow.setup.options['SIGK']
                        * (ch1space - qmax + powerflow.setup.options['SIGQ'])
                    )
                )

                ch2space = linspace(
                    start=(qmin - (powerflow.setup.options['SIGQ'] * 1e1)),
                    stop=(qmin + (powerflow.setup.options['SIGQ'] * 1e1)),
                    num=10000,
                    endpoint=True,
                )
                ch2value = 1 / (
                    ...
                    + npexp(
                        powerflow.setup.options['SIGK']
                        * (ch2space - qmin - powerflow.setup.options['SIGQ'])
                    )
                )

                chvspace = linspace(
                    start=(vesp - (powerflow.setup.options['SIGV'] * 1e1)),
                    stop=(vesp + (powerflow.setup.options['SIGV'] * 1e1)),
                    num=10000,
                    endpoint=True,
                )
                ch3value = 1 / (
                    ...
                    + npexp(
                        powerflow.setup.options['SIGK']
                        * (chvspace - vesp - powerflow.setup.options['SIGV'])
                    )
                )
                ch4value = 1 / (
                    1
                ...
                fig.savefig(
                    powerflow.setup.dirsmoothsys + "smooth-" + busname + ".png", dpi=400
                )
                plt.close(fig)
    ...
    def svcstorage(
        ...
        if powerflow.method == "CPF":
            ...
            for busname, _ in powerflow.setup.svckeys.items():
                ...
                busidxcer = powerflow.setup.dcerDF.index[
                    powerflow.setup.dcerDF['barra']
                    == powerflow.setup.dbarraDF['numero'].iloc[busidx]
                ].tolist()[0]
                ctrlbusidxcer = powerflow.setup.dbarraDF.index[
                    powerflow.setup.dbarraDF['numero']
                    == powerflow.setup.dcerDF['barra_controlada'].iloc[busidxcer]
                ].tolist()[0]
                ...
                ch1space = linspace(
                    start=(vmmax - (powerflow.setup.options['SIGV'] * 1e1)),
                    stop=(vmmax + (powerflow.setup.options['SIGV'] * 1e1)),
                    num=10000,
                    endpoint=True,
                )
                ch1value = 1 / (
                    ...
                    + npexp(
                        -powerflow.setup.options['SIGK']
                        * (ch1space - vmmax + powerflow.setup.options['SIGV'])
                    )
                )

                ch2space = linspace(
                    start=(vmmin - (powerflow.setup.options['SIGV'] * 1e1)),
                    stop=(vmmin + (powerflow.setup.options['SIGV'] * 1e1)),
                    num=10000,
                    endpoint=True,
                )
                ch2value = 1 / (
                    ...
                    + npexp(
                        powerflow.setup.options['SIGK']
                        * (ch2space - vmmin - powerflow.setup.options['SIGV'])
                    )
                )
